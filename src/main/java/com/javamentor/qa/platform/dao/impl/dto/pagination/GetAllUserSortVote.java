    ...
    public List<UserDto> getItems(Map<String, Object> param) {
        ...
        return entityManager.createQuery (
                "SELECT new com.javamentor.qa.platform.models.dto.UserDto " +
                        ...
                        "GROUP BY user.id " +
                        "ORDER BY " +
                        "((SELECT COUNT (*) FROM VoteQuestion vq WHERE vq.vote = 'UP_VOTE' AND vq.question.user.id = user.id) - " +
                        "(SELECT COUNT (*) FROM VoteQuestion vq WHERE vq.vote = 'DOWN_VOTE' AND vq.question.user.id = user.id) + " +
                        "(SELECT COUNT (*) FROM VoteAnswer va WHERE va.vote = 'UP_VOTE' AND va.answer.user.id = user.id) - " +
                        "(SELECT COUNT (*) FROM VoteAnswer va WHERE va.vote = 'DOWN_VOTE' AND va.answer.user.id = user.id)) DESC, " +
                        "user.id ASC",
                        UserDto.class)
