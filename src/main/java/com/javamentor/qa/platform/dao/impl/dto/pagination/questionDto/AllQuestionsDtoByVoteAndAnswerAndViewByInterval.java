...
@Repository("AllQuestionsDtoByVoteAndAnswerByInterval")
@RequiredArgsConstructor
public class AllQuestionsDtoByVoteAndAnswerAndViewByInterval implements PaginationDtoAble<QuestionViewDto> {
    ...
    public List<QuestionViewDto> getItems(Map<String, Object> param) {
        ...
        int interval = (int) param.get("interval") + 1;
        ...
        return entityManager.createQuery(
                        "SELECT DISTINCT new com.javamentor.qa.platform.models.dto.QuestionViewDto" +
                                ...
                                "author.fullName, author.imageLink, question.description, (SELECT COUNT(questionViewed.question.id) FROM QuestionViewed questionViewed WHERE questionViewed.question.id = question.id), " +
                                "(SELECT COUNT (*) " +
                                "FROM Answer answer " +
                                ...
                                "WHERE question.persistDateTime BETWEEN :interval AND LOCALTIMESTAMP  AND question.isDeleted = false " +
                                ...
                                "ORDER BY countVote DESC, countAnswer DESC "
                        , QuestionViewDto.class)
                .setParameter("interval", LocalDateTime.now().minusDays(interval))
