...
@Repository("QuestionPageDtoByAnswers")
@RequiredArgsConstructor
public class QuestionPageDtoByAnswers implements PaginationDtoAble<QuestionViewDto> {
    ...
    public List<QuestionViewDto> getItems(Map<String, Object> param) {
        String str = ((String) param.get("parseStr")).replace("answers:..", "");
        int countAnswers = Integer.parseInt(str);
        ...
        return entityManager.createQuery("SELECT DISTINCT new com.javamentor.qa.platform.models.dto.QuestionViewDto" +
                        ...
                        "LEFT JOIN question.user WHERE question.answers.size <= :countAnswers ORDER BY question.id", QuestionViewDto.class)
                .setParameter("countAnswers", countAnswers)
                .getResultStream()
    ...
    public int getTotalResultCount(Map<String, Object> param) {
        ...
        return Math.toIntExact((Long) entityManager.createQuery("SELECT COUNT(DISTINCT question.id) FROM Question question LEFT JOIN question.user " +
                        "WHERE question.answers.size <= :countAnswers")
                .setParameter("countAnswers", countAnswers)
                .getSingleResult());
