...
@Repository("QuestionPageDtoFromTitle")
@RequiredArgsConstructor
public class QuestionPageDtoFromTitle implements PaginationDtoAble<QuestionViewDto> {
    ...
    public List<QuestionViewDto> getItems(Map<String, Object> param) {
        String title = ((String) param.get("parseStr")).replace("title:", "");
        ...
        return entityManager.createQuery("SELECT DISTINCT new com.javamentor.qa.platform.models.dto.QuestionViewDto" +
                        ...
                        "LEFT JOIN question.user WHERE LOWER (question.title) LIKE LOWER (CONCAT('%',:titleWord,'%'))" +
                        "ORDER BY question.id", QuestionViewDto.class)
                .setParameter("titleWord", title)
                .getResultStream()
    ...
    public int getTotalResultCount(Map<String, Object> param) {
        ...
        return Math.toIntExact((Long) entityManager.createQuery("SELECT COUNT(DISTINCT question.id) FROM Question question LEFT JOIN question.user " +
                        "WHERE LOWER (question.title) LIKE LOWER (CONCAT('%',:titleWord ,'%'))")
                .setParameter("titleWord", title)
                .getSingleResult());
