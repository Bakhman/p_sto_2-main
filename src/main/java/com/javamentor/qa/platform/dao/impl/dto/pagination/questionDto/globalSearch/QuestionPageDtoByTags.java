...
@Repository("QuestionPageDtoByTags")
@RequiredArgsConstructor
public class QuestionPageDtoByTags implements PaginationDtoAble<QuestionViewDto> {
    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public List<QuestionViewDto> getItems(Map<String, Object> param) {
        String str = ((String) param.get("parseStr")).replaceAll("[\\[\\]]", "");
    ...
    public int getTotalResultCount(Map<String, Object> param) {
        ...
        return Math.toIntExact((Long) entityManager.createQuery(
                        "SELECT COUNT(*) " +
                                ...
                                "WHERE tag.name=:name " +
                                "AND question.isDeleted = false")
                .setParameter("name", str).getSingleResult());
    }
}
