    ...
    public List<QuestionViewDto> getItems(Map<String, Object> param) {
        ...
        return entityManager.createQuery(
                "SELECT DISTINCT new com.javamentor.qa.platform.models.dto.QuestionViewDto " +
                        ...
                        "(SELECT COUNT(answer.question.id) FROM Answer answer WHERE answer.question.id = question.id), " +
                        "((SELECT COUNT(*) FROM VoteQuestion voteOnQuestion WHERE voteOnQuestion.vote = 'UP_VOTE' AND voteOnQuestion.question.id = question.id) - (SELECT COUNT(*) FROM VoteQuestion voteOnQuestion WHERE voteOnQuestion.vote = 'DOWN_VOTE' AND voteOnQuestion.question.id = question.id)), " +
                        "question.persistDateTime, " +
                        "question.lastUpdateDateTime, " +
                        "(SELECT DISTINCT  CASE WHEN EXISTS (SELECT  b.id FROM BookMarks b WHERE b.user.id = author.id " +
                        "AND b.question.id = question.id) THEN true ELSE false END as isUserBookmark FROM BookMarks ) )" +
                        ...
                        "WHERE tag.id = :id " +
                        "AND question.isDeleted = false AND " +
