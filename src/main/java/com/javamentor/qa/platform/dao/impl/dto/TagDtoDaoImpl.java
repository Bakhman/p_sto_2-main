...
public class TagDtoDaoImpl implements TagDtoDao {
    ...
    public List<TagDto> getAllTags() {
        return entityManager.createQuery("SELECT new com.javamentor.qa.platform.models.dto.TagDto (tag.id,tag.name,tag.description) " +
                "FROM Tag tag", TagDto.class).getResultList();
    }

    @Override
    public void deleteIgnoredTagById(Long id, Long userId) {
        entityManager.createQuery("DELETE FROM IgnoredTag p WHERE p.ignoredTag.id=:id and p.user.id=:userId"
                ).setParameter("id", id)
    ...
    @Override
    public void deleteTrackedTagById(Long id, Long userId) {
        entityManager.createQuery("DELETE FROM TrackedTag p WHERE p.trackedTag.id=:id and p.user.id=:userId"
                ).setParameter("id", id)
                .setParameter("userId", userId)
                .executeUpdate();
    }
