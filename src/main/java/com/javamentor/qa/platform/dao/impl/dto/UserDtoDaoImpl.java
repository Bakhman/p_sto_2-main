...
public class UserDtoDaoImpl implements UserDtoDao {
    ...
    @Override
    public List<UserDto> getTopByAnswerPer(int size, int days) {
        return entityManager.createQuery(
                        "SELECT DISTINCT new com.javamentor.qa.platform.models.dto.UserDto " +
                                "(user.id, user.email, user.fullName, user.imageLink, user.city, " +
                                /*   reputation*/
                                "(SELECT COALESCE(SUM(reputation.count), 0L) FROM Reputation reputation WHERE reputation.author.id = user.id), " +
                                /* user.persistDateTime */
                                "user.persistDateTime, " +
                                ...
                                "AND a.persistDateTime BETWEEN :interval AND LOCALTIMESTAMP " +
                        ...
                        , UserDto.class)
                .setMaxResults(size)
                .setParameter("interval", LocalDateTime.now().minusDays(days))
                .getResultList();
    }
}