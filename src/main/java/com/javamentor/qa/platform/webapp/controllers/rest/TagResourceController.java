...
public class TagResourceController {
    ...
    @ApiOperation(value = "Get all tags", tags = {"GetAllTags"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "Success")})
    @GetMapping("/allTags/filter/{valueInput}")
    public ResponseEntity<List<TagDto>> getTags(@PathVariable("valueInput")String valueInput) {
        List<TagDto> list = tagDtoService.getAllTags();
        List<TagDto> filterList = list.stream()
                .filter(a->a.getName().contains(valueInput))
                .limit(5)
                .collect(Collectors.toList());
        return ResponseEntity.ok(filterList);
    }

    @ApiOperation(value = "Удаление тега из IgnoredTag", tags = {"IgnoredTag"})
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Тег успешно удален из IgnoredTag"),
            @ApiResponse(code = 400, message = "Некорректный запрос")})
    @DeleteMapping("/{id}/ignore/delete")
    @Transactional
    public ResponseEntity<?> deleteIgnoredTag(@PathVariable("id") Long id) {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getDetails();
        Long userId = user.getId();
        tagDtoService.deleteIgnoredTagById(id, userId);
        return ResponseEntity.ok().body(" successfully deleted");

    }

    @ApiOperation(value = "Удаление тега из TrackedTag", tags = {"TrackedTag"})
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Тег успешно удален из TrackedTag"),
            @ApiResponse(code = 400, message = "Некорректный запрос")})
    @DeleteMapping("/{id}/track/delete")
    @Transactional
    public ResponseEntity<?> deleteTrackedTag(@PathVariable("id") Long id) {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getDetails();
        Long userId = user.getId();
        tagDtoService.deleteTrackedTagById(id, userId);
        return ResponseEntity.ok().body(" successfully deleted");
